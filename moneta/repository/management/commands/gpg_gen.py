...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        optparse.make_option('--length', action='store', dest='length', default='2048',
        ...
        optparse.make_option('--no-existing-keys', action='store_true', dest='nokeys', default=False,
                             help='Generate keys only when no key already exists.'),
        optparse.make_option('--only-id', action='store_true', dest='onlyid', default=False,
                             help='Generate keys only when no key already exists.'),
    )
